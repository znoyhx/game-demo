import {
  ...
  worldCreationTemplateSchema,
} from '../schemas';

const buildRequest = (request: WorldCreationRequest): WorldCreationRequest =>
  worldCreationRequestSchema.parse(request);

const buildTemplate = (template: WorldCreationTemplate): WorldCreationTemplate =>
  worldCreationTemplateSchema.parse(template);
...
export const defaultWorldCreationRequest = buildRequest({
  ...
  learningGoal:
    '理解任务、战术与可解释智能系统如何在一个活着的世界里协同运作',
...
export const worldCreationTemplates = [
  buildTemplate({
...
] satisfies WorldCreationTemplate[];

export const worldTemplateRegistry = Object.freeze(
  Object.fromEntries(worldCreationTemplates.map((template) => [template.id, template])),
) as Readonly<Record<string, WorldCreationTemplate>>;

export const findWorldCreationTemplate = (templateId: string) =>
  worldTemplateRegistry[templateId] ?? null;
