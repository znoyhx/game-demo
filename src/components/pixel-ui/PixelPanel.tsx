  ...
  return (
    ...
    >
      <header className="pixel-panel__header">
        <div className="pixel-panel__heading">
          {eyebrow ? (
            <PixelLabel className="pixel-panel__eyebrow" tone="muted">
              {eyebrow}
            </PixelLabel>
          ) : null}
          <div className="pixel-panel__title-row">
            <h3 className="pixel-panel__title">{title}</h3>
            {headerAside ? <div className="pixel-panel__aside">{headerAside}</div> : null}
          </div>
          {description ? (
            <p className="pixel-panel__description">{description}</p>
          ) : null}
        </div>
      </header>
      <div className={cn('pixel-panel__body', bodyClassName)}>{children}</div>
      {footer ? <div className="pixel-panel__footer">{footer}</div> : null}
    </section>
  );
}
